...
void Face::draw(Graphics& graphics, unsigned int x, unsigned int y) {
  ...
  if (freckles) draw_part(graphics, x, y, 4 * PART_WIDTH, 0, PART_WIDTH, PART_HEIGHT);
  if (piercing) draw_part(graphics, x, y + 2 * PART_HEIGHT, 4 * PART_WIDTH, 2 * PART_HEIGHT, PART_WIDTH, PART_HEIGHT);
  if (moustache > 0) draw_part(graphics, x, y, 5 * PART_WIDTH, PART_HEIGHT * (moustache - 1), PART_WIDTH, PART_HEIGHT);
}
